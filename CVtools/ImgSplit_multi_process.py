...
"""
# This code is used to split the ori image(big resolution) to the subimages(small resolution)

# output_base = P0003__1__ 1 is for resize rate
# subimgname = outbasename + str(left) + '___' + str(up), where left->weight and up->height

# the file tree
# for example
# example--|
#          |images--|P0706.png  在splitbase()类中修改图片扩展名
#                   |P0770.png
#                   |...
#
#          |labelTxt--|P0706.txt
...
#                     |...
#
# ------------------- the output dir path(split) is optional ---------------------------
#          |split--|images--|P0706__1__0___0.png
#                           |P0706__1__0___158.png
#                           |...
#                  |labelTxt--|P0706__1__0___0.txt
#                             |P0706__1__0___158.txt
#                             |...
import os
import codecs
import numpy as np
...
class splitbase():
    def __init__(self,
        ...
        self.thresh = thresh
        self.imagepath = os.path.join(self.basepath, 'images')  # example/images
        self.labelpath = os.path.join(self.basepath, 'labelTxt')  # example/labelTxt
        self.outimagepath = os.path.join(self.outpath, 'images')  # example/split/images
        self.outlabelpath = os.path.join(self.outpath, 'labelTxt')  # example/split/labelTxt
...
if __name__ == '__main__':
    ...
    # explanation for the following parameters
    """
    splitbase(basepath = r'待分割数据集文件路径',
              outpath = r'分割后的数据及文件保存路径'
              gap = 两张被分割图片之间的重叠区域
              subsize = 分割后的图片size)
    注：一般gap设为切割图像size的20%为宜
    """
    split = splitbase(r'/home/fzh/DOTA_devkit_YOLO-master/手动挑选的数据集/',
                      r'/home/fzh/DOTA_devkit_YOLO-master/Big_Rotation_dataset/',
                      gap=150,        # 重叠区域
                      subsize=600,   # 被分割成的小图片的size
                      num_process=8
                      )
