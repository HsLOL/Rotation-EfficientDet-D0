...
# 此脚本的Anchor生成考虑到了Kmeans的结果，Kmeans的结果由ming71中的Kmeans脚本产生
import itertools
import torch
...
class Rotation_BBoxTransform(nn.Module):
    def forward(self, anchors, regression):
        ...
        temp = torch.stack([xmin, ymin, xmax, ymax, pt], dim=2)
...
if __name__ == '__main__':
    ...
    img = np.ones([4, 3, 512, 512])
    image = torch.from_numpy(img).cuda(0)
    print(f'line 185: {image.device}')
    pyramid_levels = [3, 4, 5, 6, 7]
    anchor = Anchors(pyramid_levels=pyramid_levels)

    # ratio = [(1.0, 1.0), (1.4, 0.7), (0.7, 1.4)]

    results = anchor.forward(image=image)
    print(f"line 228: {results}")
    print(f'line 185: {results.device}')
