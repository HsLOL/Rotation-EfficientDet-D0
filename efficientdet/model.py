...
class BiFPN(nn.Module):
    ...
    def __init__(self, num_channels, conv_channels, first_time=False, epsilon=1e-4, onnx_export=False, attention=True,
        ...
        Args:
            num_channels:BiFPN中的feature map的channel数
            conv_channels:the channel of the P3, P4, P5
        ...
        self.epsilon = epsilon  # 进行Fast normalized fusion 时对应的epsilon=1e-4
        self.use_p8 = use_p8

        # Conv layers
        self.conv6_up = SeparableConvBlock(num_channels, onnx_export=onnx_export)  # 在BiFPN中P6 -> P7  输出channel=88(D1)，使用swish()
        self.conv5_up = SeparableConvBlock(num_channels, onnx_export=onnx_export)  # ...     P5 -> P6
        self.conv4_up = SeparableConvBlock(num_channels, onnx_export=onnx_export)  # ...     P4 -> P5
        self.conv3_up = SeparableConvBlock(num_channels, onnx_export=onnx_export)  # ...     P3 -> P4
        self.conv4_down = SeparableConvBlock(num_channels, onnx_export=onnx_export)  # ...   P4 -> P3
        self.conv5_down = SeparableConvBlock(num_channels, onnx_export=onnx_export)  # ...   P5 -> P4
        self.conv6_down = SeparableConvBlock(num_channels, onnx_export=onnx_export)  # ...   P6 -> P5
        self.conv7_down = SeparableConvBlock(num_channels, onnx_export=onnx_export)  # ...   P7 -> P6
        if use_p8:
        ...
        所以经过类型转换这些权重(w)变成了模型的一部分，成为了模型中根据训练可以改动的参数了。
...
#         self.num_layers = num_layers  # Regressor(Box prediction net)重复的次数
#         """nn.ModuleList
#         nn.ModuleList()类似于python中的list。
#         但不同于一般的 list，加入到 nn.ModuleList 里面的 module 是会自动注册到整个网络上的\
#         同时 module 的 parameters 也会自动添加到整个网络中。
#
#         nn.ModuleList()它是一个储存不同 module，
#         并自动将每个 module 的 parameters 添加到网络之中的容器。
...
class Regressor(nn.Module):
    ...
    def __init__(self, in_channels, num_anchors, num_layers, pyramid_levels=5, onnx_export=False):
        ...
        self.conv_list = nn.ModuleList(
            [SeparableConvBlock(in_channels, in_channels, norm=False, activation=False) for i in range(num_layers)])
        self.bn_list = nn.ModuleList(
            [nn.ModuleList([nn.BatchNorm2d(in_channels, momentum=0.01, eps=1e-3) for i in range(num_layers)]) for j
             in
             range(pyramid_levels)])
        # self.header = SeparableConvBlock(in_channels, num_anchors * 4, norm=False, activation=False)
