...
# 目前该代码对应的文件结构，适用于(voc, yolo, hrsc)三种模式：
# label_path --| *.txt
#            --| *.jpg
#
# txt_save_path  --| 保存txt文件的路径
# img_save_path  --| 保存plt绘图的路径


# 自己编写dota格式，对应的文件结构如下:
# label_path --|labelTxt --| *.txt
#            --|images   --| *.jpg
#
# txt_save_path  --| 保存txt文件的路径
# img_save_path  --| 保存plt绘图的路径
...
class Kmeans:
    def __init__(self, cluster_number, all_boxes, txt_save_path=None, img_save_path=None, vis=None, save=None):
    ...
    def result2txt(self, data):
        f = open(self.txt_save_path, 'w')
        row = np.shape(data)[0]
        for i in range(row):
            ### yolo format  --> w1,h1,w2,h2
            if i == 0:
                x_y = "%d,%d" % (data[i][0], data[i][1])
            else:
                x_y = ", %d,%d" % (data[i][0], data[i][1])

            ### other format --> 输出结果w1 h1\n w2 h2\n ...
            # x_y = "%d  %d \n" % (data[i][0], data[i][1])
            f.write(x_y)
        f.close()
    ...
    def anchor_clusters(self, size=None):
        ...
        if self.img_save_path:
            plt.savefig(os.path.join(img_save_path, 'anchor_clusters.jpg'))


        #####  Method 2 : 自己写一遍kmeans，这个的iou更高，推荐使用
        ...
        # print("K anchors:\n {}\n".format(result))
        # print("Accuracy: {:.2f}%\n".format(self.avg_iou(boxes, result) * 100))
        # self.Avg_IoU_result = self.avg_iou(boxes, result) * 100
        #
        # if self.txt_save_path:
        #     self.result2txt(result)
        #
        #     ## 聚类结果分析(仅支持yolo格式的anchor文本)
        #     with open(self.txt_save_path, 'r') as f:
        #         contents = f.read()
        ...
        #         print('[Anchor聚类后的长宽比(ratio)]:\n{}\n\n[Anchor聚类后的面积(area)]:\n{}\n'.format(ratio, area))
        #         print('[Anchor聚类后的面积开根号sqrt(area)]:\n{}'.format(squre))
    ...
    def area_cluster(self):
        ...
        if self.save:
            plt.savefig(os.path.join(self.img_save_path, 'area_cluster.jpg'))
    ...
    def ratio_cluster(self, vis=False):
        ...
        if self.vis:
            ...
            for c in centroids:
                ...
                plt.scatter(int(0.5 * len(boxes)), c.item(), marker='*', c='red', s=100)
        if self.save:
            plt.savefig(os.path.join(self.img_save_path, 'ratio_cluster.jpg'))
...
def get_all_boxes(path, mode=None, resize=False):
    ...
    if mode == 'voc':
        ...
        return_ratio = True  # 只返回比例,供数据分析用
        labels = sorted(glob.glob(os.path.join(path, 'labelTxt/', '*.txt*')))
        for label in tqdm(labels, desc='Loading labels'):
            img_path = os.path.join(path, 'images/', os.path.split(label)[1][:-4] + '.png')
            height, width, _ = cv2.imread(img_path).shape
            with open(label, 'r') as f:
                contents = f.read()
                lines = contents.split('\n')
                ...
                for object in lines:
                    ...
                    if not return_ratio:
                        ...
                        # 使用旋转目标检测(x_c, y_c, w, h, theta, cls, diff)
                        box_w = float(coors[2])
                        box_h = float(coors[3])
                        boxes.append((box_w, box_h))
    else:
...
if __name__ == "__main__":
    ksize = 8  # 画'elbow curve'找到Kmeans算法使用几个种子个数合适
    cluster_number = 4  # 种子点个数
    vis = True  # 可视化聚类结果
    save = True  # 是否将可视化的结果进行保存
    resize = 512  # 长边缩放到多大尺寸再聚类
    label_path = '/home/fzh/Kmeans/dataset'
    txt_save_path = '/home/fzh/Kmeans/dataset/result.txt'
    img_save_path = r'/home/fzh/Kmeans/dataset/savefig'
    # all_boxes:存储所有.txt文件中实例的长度和宽度(先宽度后长度)
    all_boxes = get_all_boxes(label_path, 'dota', resize=resize)

    # 绘制elbow curve
    # result = {}
    # for num in range(2, ksize):
    #     kmeans = Kmeans(num, all_boxes,
    #                     txt_save_path=txt_save_path,
    #                     img_save_path=img_save_path,
    #                     vis=False,
    #                     save=False)
    #     kmeans.anchor_clusters()
    #     result[num] = kmeans.Avg_IoU_result
    #
    # plt.figure('anchor cluster elbow curve')
    # plt.plot(np.arange(2, ksize),
    #          [result[k] for k in range(2, ksize)], "o-")
    # plt.title("anchor cluster elbow curve")
    # plt.ylabel("mean IOU")
    # plt.xlabel("N clusters (= N anchor boxes)")
    # plt.savefig(os.path.join(img_save_path, 'anchor-cluster-elbow-curve.jpg'))
