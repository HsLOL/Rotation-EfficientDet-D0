...
# 防止由于对图像进行resize后，图片中边界框的位置也发生变化
...
# for coco-eval
def eval_preprocess(*image_path, max_size=512, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)):
    ori_imgs = [cv2.imread(img_path) for img_path in image_path]
    normalized_imgs = [(img[..., ::-1] / 255 - mean) / std for img in ori_imgs]
    imgs_meta = [aspectaware_resize_padding(img, max_size, max_size,
                                            means=None) for img in normalized_imgs]
...
def postprocess(x, anchors, regression, classification, regressBoxes, clipBoxes, addBoxes, threshold, iou_threshold):
    transformed_anchors = regressBoxes(anchors, regression)  # 根据anchor和regression的预测偏移量生成预测框
    transformed_anchors = clipBoxes(transformed_anchors, x)  # 将生成的预测框超出图片区域的进行裁剪
    ...
    for i in range(x.shape[0]):
        ...
        transformed_bboxes = addBoxes(transformed_anchors_per, scores_per)
