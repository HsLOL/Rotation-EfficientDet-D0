...
def read_anno_and_output_file(args):
    ...
    for idx, txtfile in (enumerate(txt_filelist)):
...
def drawOPENCVformat(args):
    ...
    img = cv2.imread(os.path.join(args.root_path, 'images', args.visualize_name + '.png'))
    with open(os.path.join(args.output_path, 'labelTxt', args.visualize_name + '.txt'), 'r') as f_in:
        lines = f_in.readlines()
        head = lines[:2]
        body = lines[2:]
        content = []
        ...
        # check theta
        print(f'---- 检查经过编写函数转换后{args.visualize_name}的OPENCV格式是否含有角度问题 ----')
        ...
        cv2.drawContours(image=img,
                         ...
                         color=[0, 0, 255],
                         thickness=2)
        cv2.imwrite('Draw_Opencv.jpg', img)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--root_path', type=str,
                        default=r'/home/fzh/DOTA_devkit_YOLO-master/DOTA_demo/')

    parser.add_argument('--output_path', type=str,
                        default=r'/home/fzh/DOTA_devkit_YOLO-master/add_funcion_output/')

    parser.add_argument('--visualize_name', type=str,
                        default='P0003')
